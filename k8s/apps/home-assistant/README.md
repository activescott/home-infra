...
I have a Honeywell "Vista 20P" connected to a variety of security sensors at parameter and inside the home and garage. Specifically the chip on the board says `WA20P-9.12`. I coonnect that to a [AlarmDecoder hardware/bridge](https://www.alarmdecoder.com/wiki/index.php/Getting_Started) on a Raspberry Pi (sitting inside the alaram box). This allows browser and API-based control of the Alarm Panel. The [AlarmDecoder Home Assistant Integration](https://www.home-assistant.io/integrations/alarmdecoder/) is then a plugin for Home Assistant that fully controls alarm and all the sensors in Home Assistant. This gives fully supported motion sensors and perimeter sensors for the entire house to Home Assistant and those sensors are cheaper than Z-Wave or Zigbee on eBay. Works perfect for a couple years now. The Honeywell is a bit wonky to program (a series of number sequences, but its not hard, just tedious one-time setup).
