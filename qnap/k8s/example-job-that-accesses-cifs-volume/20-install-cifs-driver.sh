...
helm install csi-driver-smb csi-driver-smb/csi-driver-smb --namespace kube-system --version $DRIVER_VERSION

echo "installing csi-driver-smb driver complete."
echo "Monitoring relevant pods. Press Ctrl+C to stop:"
kubectl --namespace=kube-system get pods --selector="app in (csi-smb-controller, csi-smb-node)" --watch
